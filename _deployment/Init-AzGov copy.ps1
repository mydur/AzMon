...
| ConvertFrom-Json
# AZAUTO
Write-Host ("Service Principal: " + $AzAutoAadaName) -ForegroundColor "White"
$AzAutoAada = (az ad sp create-for-rbac `
                --name "$AzAutoAadaName" `
                --years 3) `
| ConvertFrom-Json
Write-Host ($AzAutoAada.displayName + ": " + $AzAutoAada.appId) -ForegroundColor "White"
...
| ConvertFrom-Json
Write-Host ($AzAutoAada.displayName + " keyvault secret: " + $AzAutoAadaKeyv.id) -ForegroundColor "White"
